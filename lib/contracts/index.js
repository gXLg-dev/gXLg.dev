...
const terms = require("../../static/legal/terms/list.json");
...
const user_to_stripe_customer = new BadTable("./db/user_to_stripe_customer.db", {
  "key": "id",
  "values": [
    { "name": "id", "maxLength": 36 },
    { "name": "stripe_id", "maxLength": 255 }
  ]
});
...
async function propose_contract(user, name, develop, host, lang) {
  ...
  const customer = await user_to_stripe_customer[user](async (e, c) => {
    if (c.exists()) return e.stripe_id;
...
async function get_url(id) {
  return await contracts[id](async (c, t) => {
    if (!t.exists()) return { "done": false };
    if (c.status != STATUS_OPEN) return { "done": true };
    const sess = await stripe.checkout.sessions.retrieve(c.stripe_id);
    if (sess.status == "complete") return { "done": true };
    if (sess.url == null) return { "done": false };
    return { "done": false, "url": sess.url };
  });
}
...
async function update_contract(cid, c, t) {
  let sid = c.stripe_id;
  let { status, started_at, cancelled_at } = c;

  if (status == STATUS_OPEN) {
    const sess = await stripe.checkout.sessions.retrieve(sid);

    if (sess.status == "expired") {
      await contracts_list(
        e => e.splice(e.findIndex(i => i.id == cid), 1)
      );
      await user_to_contracts[c.user](
        e => e.splice(e.findIndex(i => i.id == cid), 1)
      );
      t.remove();
      return;
    ...
    if (sess.status == "complete") {
      ...
      await sendMail(email, "{{email.contract.subject1}}", "{{email.contract.subject1}}", [
        ...
        ["n", (c.host == 0 ? "{{email.contract.paid}}: " : "{{email.contract.start}}: ") + (new Date(parseInt(started_at)).toLocaleString(c.lang))],
        ...
        "terms.pdf": "./static/legal/terms/" + c.terms + "/" + c.lang + ".pdf",
        "withdrawal.pdf": "./static/legal/cancel/" + c.lang + ".pdf"
      });
    }
  }
  if (status == STATUS_RUNNING) {
    ...
    if (sub.status == "canceled") {
      ...
      await sendMail(email, "{{email.contract.subject2}}", "{{email.contract.subject2}}", [
        ...
        ["n", "{{email.contract.dev:" + (c.dev / 100).toFixed(2) + "}}"],
        ["n", "{{email.contract.host:" + (c.host / 100).toFixed(2) + "}}"],
        ["n", "{{email.contract.start}}: " + (new Date(parseInt(started_at)).toLocaleString(c.lang))],
        ["n", "{{email.contract.end}}: " + (new Date(parseInt(cancelled_at)).toLocaleString(c.lang))],
...
const polling = setInterval(async () => {
  ...
  await Promise.all(cts.map(async ct => {
    ...
    await contracts[cid](async (c, t) => {
      try {
        await update_contract(cid, c, t);
      } catch (err) {
        console.log("Error during contract polling: ", err);
      }
    });
  }));
}, 10 * 60 * 1000);
