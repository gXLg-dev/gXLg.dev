...
<tr><td style="font-size: 26px;">{title}</td></tr>
{body}
</table>
...
const convert = {
  "": t => "<tr><td>" + t + "</td><tr>",
  "c": t => "<tr><td style=\"font-family: 'Courier New', 'Droid Sans Mono', monospace;\"><table bgcolor='#f9ae77' cellpadding='5'><tr><td>" + t + "</td></tr></table></td><tr>",
...
async function sendMail(recv, subject, title, body) {
  const t = text
    .replace("{title}", title)
    .replace("{body}", body.map(b => b[1]).join("\n"));

  const h = html
    .replace("{title}", title)
    .replace("{body}", body.map(b => convert[b[0]](b[1])).join("\n"));
